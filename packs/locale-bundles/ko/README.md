...
```bash
python tools/build_locale_pack_charset.py --pack-root packs/locale-bundles/ko --font-pack-id ko-cjk
```
...
python tools/generate_binfont_with_lv_font_conv.py `
  ...
  --charset-file packs/locale-bundles/ko/fonts/ko-cjk/charset.txt `
  --output packs/locale-bundles/ko/fonts/ko-cjk/font.bin `
  --size 16 `
  --bpp 2 `
