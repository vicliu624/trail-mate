...
bool MeshtasticBleService::handleToRadioPacket(meshtastic_MeshPacket& packet)
{
    ...
    if (packet.decoded.portnum == meshtastic_PortNum_TEXT_MESSAGE_APP)
    {
        ...
        const chat::MessageId msg_id = ctx_.getChatService().sendText(channel, text, packet.to);
...
bool MeshtasticBleService::handleAdmin(meshtastic_MeshPacket& packet)
{
    ...
    case meshtastic_AdminMessage_get_device_connection_status_request_tag:
        ...
        resp.get_device_connection_status_response.bluetooth.is_connected = Bluefruit.connected();
        has_resp = true;
        break;
    ...
    {
        ::boards::gat562_mesh_evb_pro::Gat562Board::instance().setCurrentEpochSeconds(
            static_cast<uint32_t>(req.set_time_only));
        return true;
    }
...
bool MeshtasticBleService::handleLocalSelfPacket(meshtastic_MeshPacket& packet)
{
...
void MeshtasticBleService::pumpIncomingAppData()
{
    chat::IMeshAdapter* adapter = ctx_.getMeshAdapter();
    if (!adapter)
    {
...
bool MeshtasticBleService::popToPhone(Frame* out)
{
    if (!out)
    {
        return false;
...
bool MeshtasticBleService::encodeFromRadio(const meshtastic_FromRadio& from, uint32_t from_num, Frame* out) const
{
    if (!out)
    {
        return false;
...
void MeshtasticBleService::enqueueQueueStatus(uint32_t packet_id, bool ok)
{
...
void MeshtasticBleService::enqueueConfigSnapshot(uint32_t config_nonce)
{
    meshtastic_FromRadio from = meshtastic_FromRadio_init_zero;
    from.which_payload_variant = meshtastic_FromRadio_my_info_tag;
    from.my_info = buildMyInfo();
    enqueueFromRadio(from, config_nonce);
    ...
    if (const auto* store = ctx_.getNodeStore())
    {
        ...
        for (const auto& entry : entries)
        {
            ...
            enqueueFromRadio(from, entry.node_id);
        }
    }
    ...
    for (uint8_t channel_idx = 0; channel_idx < kMaxMeshtasticChannels; ++channel_idx)
    {
        from = meshtastic_FromRadio_init_zero;
        from.which_payload_variant = meshtastic_FromRadio_channel_tag;
        from.channel = buildChannel(channel_idx);
        enqueueFromRadio(from, config_nonce);
    }

    const meshtastic_AdminMessage_ConfigType config_types[] = {
    ...
    for (const auto config_type : config_types)
    {
        ...
        from.config = buildConfig(config_type);
        enqueueFromRadio(from, config_nonce);
    }

    const meshtastic_AdminMessage_ModuleConfigType module_types[] = {
    ...
    for (const auto module_type : module_types)
    {
        ...
        from.moduleConfig = buildModuleConfig(module_type);
        enqueueFromRadio(from, config_nonce);
    }

    from = meshtastic_FromRadio_init_zero;
    from.which_payload_variant = meshtastic_FromRadio_config_complete_id_tag;
    from.config_complete_id = config_nonce;
    enqueueFromRadio(from, config_nonce);

    notifyFromNum(config_nonce);
}

void MeshtasticBleService::enqueueFromRadio(const meshtastic_FromRadio& from, uint32_t from_num)
{
    Frame frame{};
...
meshtastic_MyNodeInfo MeshtasticBleService::buildMyInfo() const
{
    ...
    const uint32_t addr0 = NRF_FICR->DEVICEADDR[0];
    const uint32_t addr1 = NRF_FICR->DEVICEADDR[1];
    const std::array<uint8_t, 6> mac =
        platform::nrf52::arduino_common::device_identity::deriveMacAddressFromDeviceAddress(addr0, addr1);
    std::memcpy(info.device_id.bytes, mac.data(), mac.size());
    std::memcpy(info.device_id.bytes + mac.size(), &info.my_node_num, sizeof(info.my_node_num));
    info.device_id.size = static_cast<pb_size_t>(mac.size() + sizeof(info.my_node_num));
...
meshtastic_NodeInfo MeshtasticBleService::buildSelfNodeInfo() const
{
...
meshtastic_NodeInfo MeshtasticBleService::buildNodeInfoFromEntry(const chat::contacts::NodeEntry& entry) const
{
    meshtastic_NodeInfo info = meshtastic_NodeInfo_init_zero;
...
meshtastic_DeviceMetadata MeshtasticBleService::buildMetadata() const
{
...
meshtastic_DeviceMetrics MeshtasticBleService::buildDeviceMetrics() const
{
...
meshtastic_LocalStats MeshtasticBleService::buildLocalStats() const
{
...
meshtastic_DeviceUIConfig MeshtasticBleService::buildDeviceUi() const
{
...
meshtastic_Channel MeshtasticBleService::buildChannel(uint8_t idx) const
{
...
meshtastic_Config MeshtasticBleService::buildConfig(meshtastic_AdminMessage_ConfigType type) const
{
    const auto& cfg = ctx_.getConfig();
    ...
    case meshtastic_AdminMessage_ConfigType_BLUETOOTH_CONFIG:
        ...
        out.payload_variant.bluetooth.mode = meshtastic_Config_BluetoothConfig_PairingMode_NO_PIN;
        out.payload_variant.bluetooth.fixed_pin = 0;
        break;
...
meshtastic_ModuleConfig MeshtasticBleService::buildModuleConfig(meshtastic_AdminMessage_ModuleConfigType type) const
{
...
meshtastic_MeshPacket MeshtasticBleService::buildPacketFromText(const chat::MeshIncomingText& msg) const
{
...
meshtastic_MeshPacket MeshtasticBleService::buildPacketFromData(const chat::MeshIncomingData& msg) const
{
    meshtastic_MeshPacket packet = meshtastic_MeshPacket_init_zero;
    packet.from = msg.from;
    packet.to = msg.to;
