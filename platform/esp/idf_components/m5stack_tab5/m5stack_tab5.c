...
esp_err_t bsp_i2c_scan()
{
    ...
    if (!bsp_i2c_lock(1000)) {  // 1 second timeout
...
#define GPIO_SDMMC_DET  (GPIO_NUM_NC)  // SDIO 卡检�?
// M5Stack-Tab5-P4
...
void bsp_codec_init(void)
{
    ...
    if (!bsp_i2c_lock(2000)) {  // 2 second timeout for initialization
    ...
    codec_cfg->i2s_read            = bsp_i2s_read;            // I2S 读数�?
    codec_cfg->i2s_write           = bsp_i2s_write;           // I2S 写数�?
...
static void lvgl_read_cb(lv_indev_t* indev, lv_indev_data_t* data)
{
    ...
    if (!bsp_i2c_lock(0)) {  // Try-lock: if bus is busy, skip this read cycle
