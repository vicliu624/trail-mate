...
Tab5CodecCompat::~Tab5CodecCompat()
{
    if (scratch_)
    {
        std::free(scratch_);
...
bool Tab5CodecCompat::ensure_ready()
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
...
bool Tab5CodecCompat::ensure_scratch(size_t size)
{
...
int Tab5CodecCompat::open(uint8_t bits_per_sample, uint8_t channel, uint32_t sample_rate)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    if (!ensure_ready())
    {
...
void Tab5CodecCompat::close()
{
    setOutMute(true);
}

int Tab5CodecCompat::write(uint8_t* buffer, size_t size)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    if (!ensure_ready())
    {
...
int Tab5CodecCompat::read(uint8_t* buffer, size_t size)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    if (!ensure_ready())
    {
...
void Tab5CodecCompat::setMute(bool enable)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    mute_ = enable;
    if (ensure_ready())
...
bool Tab5CodecCompat::getMute() const
{
    return mute_;
}

void Tab5CodecCompat::setOutMute(bool enable)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    out_mute_ = enable;
    if (ensure_ready())
...
bool Tab5CodecCompat::getOutMute() const
{
    return out_mute_;
}

void Tab5CodecCompat::setVolume(uint8_t level)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    volume_ = level;
    if (ensure_ready())
...
int Tab5CodecCompat::getVolume() const
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    return volume_;
#else
...
void Tab5CodecCompat::setGain(float db_value)
{
#if defined(TRAIL_MATE_ESP_BOARD_TAB5)
    gain_db_ = db_value;
    if (ensure_ready())
...
float Tab5CodecCompat::getGain() const
{
    return gain_db_;
}

bool Tab5CodecCompat::ready() const
{
    return ready_;
}
