# `platform/esp/boards/tab5`

Planned home for Tab5 board support and board-specific adapters.
...
- become the point where `AppContext` is bound into the Tab5 IDF app shell
- replace the current placeholder handle resolution with real `BoardBase` / `GpsBoard` adapters
- keep Tab5-specific pin/capability knowledge in `tab5_board_profile.h` rather than scattering it into shell entry code
