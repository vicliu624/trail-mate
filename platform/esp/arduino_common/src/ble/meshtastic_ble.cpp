...
{
  public:
    explicit PhoneApi(MeshtasticBleService& service, app::IAppBleFacade& ctx)
        : service_(service),
          ctx_(ctx)
    {
  ...
  private:
    ...
    chat::meshtastic::MtAdapter* meshtasticBackend() const
    {
        ...
        return static_cast<chat::meshtastic::MtAdapter*>(
            adapter->backendForProtocol(adapter->backendProtocol()));
    }
    ...
    bool handleToRadioPacket(meshtastic_MeshPacket& packet)
    {
        ...
        if (adapter && adapter->backendProtocol() == chat::MeshProtocol::Meshtastic)
        {
            if (auto* backend = adapter->backendForProtocol(adapter->backendProtocol()))
            {
