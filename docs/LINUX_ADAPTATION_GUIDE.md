...
| CI | 中等偏好 | `.github/workflows/cardputer-zero-linux.yml` 构建 simulator、tests、rpi CMake shell，跑边界检查和 WSL-style script | 不构建 M5 SDK path，不跑真机/虚拟 fbdev，不验证 Docker GUI path |
...
专门的 `Cardputer Zero Linux` workflow 会跑边界检查、Linux simulator build、smoke tests、Pi OS CMake shell build 和 WSL-style validation。
...
cmake --build --preset linux-test-debug-build
...
```powershell
powershell -ExecutionPolicy Bypass -File .\apps\linux_sim\scripts\run-dev-container.ps1
```
...
```bash
cd apps/linux_rpi
export TRAIL_MATE_M5STACK_LINUX_LIBS_PATH=/path/to/M5Stack_Linux_Libs
