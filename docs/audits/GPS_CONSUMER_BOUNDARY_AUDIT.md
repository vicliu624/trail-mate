...
| LVGL GPS/status screens | `modules/ui_shared` screens and dashboard widgets include `platform/ui/gps_runtime.h`. | `GpsStatusModel` / `DeviceStatusModel` backed by `ILocationSource`, `ITimeAuthority`, and snapshots. | Legacy UI compatibility surface | Phase 5 |
| LVGL settings GPS controls | `modules/ui_shared/src/ui/screens/settings/settings_page_components.cpp` calls `gps_runtime` setters for receiver, GNSS, interval, power strategy, and external NMEA settings. | `SettingsModel` command sink backed by config patches and platform GPS runtime shell. | Legacy UI compatibility surface | Phase 5 / later Config Core |
