...
| legacy app implementation roots | contained historical roots | targeted root deletion when builds no longer need them |
...
- new product behavior inside `legacy/app_implementations`
...
documents, enforces final app shell directory semantics, and blocks regression
to burned-down legacy headers or renderer-level UX selection.
