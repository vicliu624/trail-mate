...
{
    bool function_touch = false;
    bool any_activity = false;
};

class TImpulsePlusBoard final : public BoardBase
{
  public:
    class I2cGuard
    {
      public:
        explicit I2cGuard(TImpulsePlusBoard& board, uint32_t timeout_ms = 100);
        ~I2cGuard();
      ...
      private:
        TImpulsePlusBoard* board_ = nullptr;
        bool locked_ = false;
    };

    static TImpulsePlusBoard& instance();
    ~TImpulsePlusBoard() override;
    ...
    uint32_t inputLongPressMs() const;
    ...
    void clearDisplay();
    void drawDisplayText(int x, int y, const char* text, uint8_t size = 1, bool inverse = false);
  ...
  private:
    ...
    uint8_t message_tone_volume_ = 0;
