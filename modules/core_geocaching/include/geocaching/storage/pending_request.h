...
{
enum class PendingRequestResult : uint8_t { Ready, None, Corrupt };
                                                   ...
                                                   ByteView key, ByteView value, PendingRequestView& out)
{
    ...
    ByteView task_bytes; TaskView task;
    ...
    {
        ByteView head_bytes; CacheHeadView head;
    ...
    out.request = outgoing.request; out.task_id = outgoing.task_id;
                                               ...
                                               ByteView after_key, PendingRequestView& out)
{
    ...
    while (state.next(cursor, entry))
    {
        ...
        if (result == PendingRequestResult::Corrupt) { out = {}; return result; }
