...
int main()
{
    ...
    auto accept = [](const LogicalState::View&) { return true; };
    if (!state.apply(&initial, 1, accept)) return 1;
    MutationView changes[] = {{5, {&key, 1}, {}, true}, {5, {&other_key, 1}, {&value, 1}, false}};
    if (state.apply(changes, 2, [](const LogicalState::View&) { return false; })) return 2;
    ...
    uint8_t large[32]{}; MutationView overflow{5, {&key, 1}, {large, sizeof(large)}, false};
