...
int main(int argc, char** argv)
{
    ...
    protocol::CmpReader reader({data.data(), data.size()}); size_t fields = 0; ByteView encoded;
    if (!reader.array(fields, 2) || !reader.binary(encoded, 4096)) return 2;
    RecordView record;
    if (!protocol::decodeGeocacheRecord(encoded, record)) return 3;
    std::array<uint8_t, 4096> output{}; size_t written = 0;
    ...
    record.name = "A"; record.longitude_e7 = 1800000000;
