...
int main()
{
    ...
    storage::CacheHeadView head; head.install_generation = 1;
    ...
    head.install_generation = 1; record.generation = 2;
    if (storage::classifyInstallRecovery({cache, 32}, head, record) != Action::Inconsistent) return 5;
    record.generation = 1; record.phase = storage::InstallPhase::ConflictExternal;
