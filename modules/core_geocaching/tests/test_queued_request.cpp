...
int main()
{
    ...
    RequestId request_id; request_id.bytes[0] = 1;
    Destination local, remote; local.bytes[0] = 2; remote.bytes[0] = 3;
    std::array<uint8_t, 16> task_id{}; task_id[0] = 4;
    uint8_t request[128]{}; size_t request_size = 0;
    ...
    std::array<uint8_t, 9000> output{}; size_t size = 0;
    storage::StoredTime time; time.boot_id[0] = 5;
    ...
    protocol::CmpReader outgoing(mutations[0].value); size_t count = 0; ByteView stored_request;
    ...
    if (storage::encodeNewRequestTask(7, local, remote, request_id, task_id, 3,
                                      {request, request_size}, time, *scratch, output.data(), output.size(), size) || size) return 5;
    return 0;
}
