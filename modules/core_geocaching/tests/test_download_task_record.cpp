...
int main()
{
    ...
    RequestId request_id; Destination local, remote; GeocacheId cache; RevisionHash hash;
    std::array<uint8_t, 16> task{};
    uint8_t request[256]{}; size_t request_size = 0;
    ...
    protocol::CmpReader record(mutations[1].value); size_t fields = 0; uint64_t kind = 0; ByteView stored_id, stored_hash;
