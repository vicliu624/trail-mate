...
int SdRuntimeFile::read(void* out, size_t count)
{
    ...
    std::memcpy(out, bytes.data() + impl_->offset, n); impl_->offset += n; return static_cast<int>(n);
}
}
struct Digest
{
    ...
    bool finalize(uint8_t* out, size_t) { chat::reticulum::fullHash(data.data(), data.size(), out); return true; }
};
int main()
{
    using namespace platform::esp::arduino_common::geocaching;
    Digest digest; SdGpxHash<Digest> reader(digest);
    ...
    for (unsigned i = 0; i < 3; ++i) if (reader.step() != GpxHashStep::Reading) return 2;
    if (reader.step() != GpxHashStep::Complete || !reader.result(actual) || actual != expected) return 3;
    Digest growing_digest; SdGpxHash<Digest> growing(growing_digest);
    if (!growing.open("local.gpx")) return 4;
    for (unsigned i = 0; i < 3; ++i) if (growing.step() != GpxHashStep::Reading) return 5;
    bytes.push_back(9);
