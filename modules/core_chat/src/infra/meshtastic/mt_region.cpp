...
float computeFrequencyMhz(const RegionInfo* region, float bw_khz, const char* channel_name)
{
    ...
    const float spacing_khz = region->spacing_khz > 0.0f ? region->spacing_khz : 0.0f;
    const float spacing_mhz = spacing_khz / 1000.0f;
    ...
    const uint32_t channel_num = djb2Hash(channel_name) % num_channels;
...
float estimateFrequencyMhz(uint8_t region_code, uint8_t modem_preset)
{
    ...
    const meshtastic_Config_LoRaConfig_ModemPreset preset =
