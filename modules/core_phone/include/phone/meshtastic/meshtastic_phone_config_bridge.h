...
namespace phone::meshtastic::config_bridge
{
...
inline void repairLegacyMqttConfig(meshtastic_LocalModuleConfig* config)
{
    ...
    {
        copyBounded(config->mqtt.address, sizeof(config->mqtt.address), defaults::kDefaultMqttAddress);
    }
    if (config->mqtt.username[0] == '\0')
    {
        copyBounded(config->mqtt.username, sizeof(config->mqtt.username), defaults::kDefaultMqttUsername);
    }
    if (config->mqtt.password[0] == '\0')
    {
        copyBounded(config->mqtt.password, sizeof(config->mqtt.password), defaults::kDefaultMqttPassword);
    }
    if (config->mqtt.root[0] == '\0')
    {
        copyBounded(config->mqtt.root, sizeof(config->mqtt.root), defaults::kDefaultMqttRoot);
    }
    if (!config->mqtt.has_map_report_settings)
    {
        config->mqtt.has_map_report_settings = true;
        config->mqtt.map_report_settings.publish_interval_secs = defaults::kDefaultMapPublishIntervalSecs;
...
inline void initDefaultModuleConfig(meshtastic_LocalModuleConfig* config, uint32_t self_node)
{
    ...
    *config = zero;
    config->version = defaults::kModuleConfigVersion;
    config->has_mqtt = true;
    ...
    config->mqtt.encryption_enabled = defaults::kDefaultMqttEncryptionEnabled;
    config->mqtt.tls_enabled = defaults::kDefaultMqttTlsEnabled;
    config->mqtt.has_map_report_settings = true;
    config->mqtt.map_report_settings.publish_interval_secs = defaults::kDefaultMapPublishIntervalSecs;
    config->mqtt.map_report_settings.position_precision = 0;
    config->mqtt.map_report_settings.should_report_location = false;
    ...
    config->detection_sensor.minimum_broadcast_secs = defaults::kDefaultDetectionMinBroadcastSecs;

    config->ambient_lighting.current = defaults::kDefaultAmbientCurrent;
