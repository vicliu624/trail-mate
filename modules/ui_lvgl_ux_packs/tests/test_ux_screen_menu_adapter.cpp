...
int main()
{
    ...
    assert(menu.size() == 3);
    assert(menu.items()[0].screen_id == ui_lvgl_ux::ScreenId::Dashboard);
    assert(menu.items()[1].screen_id == ui_lvgl_ux::ScreenId::Chat);
    assert(menu.items()[2].screen_id == ui_lvgl_ux::ScreenId::Settings);
