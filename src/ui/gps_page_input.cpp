    ...
    if (target == g_gps_state.pan_v_indicator) {
        ...
        if (code == LV_EVENT_KEY && g_gps_state.pan_v_editing) {
            ...
            GPS_LOG("[GPS] PanV KEY: key=%d, step=%d, pan_y: %d -> %d\n", 
                    key, step, g_gps_state.pan_y, g_gps_state.pan_y + step * MAP_PAN_STEP);
            g_gps_state.pan_y += step * MAP_PAN_STEP;
            g_gps_state.pending_refresh = true;
            if (g_gps_state.map != NULL) {
                lv_obj_invalidate(g_gps_state.map);
            }
...
static void handle_click(lv_obj_t* target) {
    ...
    switch (id) {
        ...
        case ControlId::PanHBtn:
            if (g_gps_state.pan_h_editing) {  // Use pan_h_editing to match original
                action_pan_exit();
            } else {
        ...
        case ControlId::PanVBtn:
            if (g_gps_state.pan_v_editing) {  // Use pan_v_editing to match original
                action_pan_exit();
            } else {
...
static void action_position_center() {
    ...
    GPS_LOG("[GPS] Position action: centered GPS marker\n");
}
...
static void action_pan_exit() {
    if (g_gps_state.edit_mode == 1) {  // PanH
        exit_pan_h_mode();
    } else if (g_gps_state.edit_mode == 2) {  // PanV
        exit_pan_v_mode();
    }
}
...
void zoom_popup_handle_rotary(int32_t diff) {
    ...
    if (diff > 0) {
        if (g_gps_state.popup_zoom < MAX_ZOOM) {
            g_gps_state.popup_zoom++;
            zoom_changed = true;
        }
    } else if (diff < 0) {
        if (g_gps_state.popup_zoom > MIN_ZOOM) {
            g_gps_state.popup_zoom--;
            zoom_changed = true;
        }
    }
