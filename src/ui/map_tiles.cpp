...
static void load_tile_image(TileContext& ctx, MapTile &tile)
{
    ...
    // If tile already has object, it's already loaded
    if (tile.img_obj != NULL) {
        tile.last_used_ms = millis();
        GPS_LOG("[GPS] load_tile_image: Tile %d/%d/%d already has object\n", tile.z, tile.x, tile.y);
        return;
    }
...
static void layout_loaded_tile_objects(TileContext& ctx)
{
    ...
    for (auto &tile : *ctx.tiles) {
        ...
        if (is_visible) {
            ...
            if (tile.img_obj == NULL) {
                // Create placeholder label immediately for visible tiles
                // Note: load_tile_image will calculate screen position internally,
                // but we already have it, so we'll update it after loading
                load_tile_image(ctx, tile);
                // Update position with the correct screen coordinates we calculated
                if (tile.img_obj != NULL) {
                    lv_obj_set_pos(tile.img_obj, screen_x, screen_y);
                    // Ensure visibility flag is correct (load_tile_image may have set it differently)
                    tile.visible = is_visible;
                }
            } else {
...
static void evict_cache(TileContext& ctx)
{
    ...
    if (ctx.tiles->size() > TILE_RECORD_LIMIT) {
        ...
        ctx.tiles->erase(std::remove_if(ctx.tiles->begin(), ctx.tiles->end(),
            [](const MapTile &t) { return t.record_evicted; }), ctx.tiles->end());
    }
}
...
void tile_loader_step(TileContext& ctx)
{
    ...
    if (best != nullptr) {
        ...
        lv_obj_invalidate(ctx.map_container);
